...
export default function AdminIntegracaoInterna() {
  ...
  const [dados, setDados] = useState<IntegracaoData[]>([
    {
      id: 1,
      nome: "Exemplo 1",
      status: "Exemplo 2",
      ultimaSincronizacao: "Exemplo 3",
      erros: "Exemplo 4",
      tipo: "Exemplo 5",
    },
    {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (editingId) {
      ...
      const novoRegistro: IntegracaoData = {
  ...
  const handleEdit = (item: IntegracaoData) => {
    setFormData({
      nome: item.nome,
      status: item.status,
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-slate-100 p-4 md:p-8">
      ...
      <div className="mb-8">
        <div className="flex items-center gap-3 mb-2">
          ...
          <h1 className="text-3xl font-bold text-slate-900">Integração Interna</h1>
        </div>
        <p className="text-slate-600">Gerenciamento de integrações internas do sistema</p>
      </div>
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-slate-200">
                    ...
                    <th className="text-left py-3 px-4 font-semibold text-slate-700">Erros</th>
      ...
      <td className="py-3 px-4">{item.erros}</td>
