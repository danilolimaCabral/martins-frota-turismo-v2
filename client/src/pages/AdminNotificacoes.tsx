...
import { Bell, Plus, Search, Edit, Trash2 , ArrowLeft } from "lucide-react";
...
export default function AdminNotificacoes() {
  ...
  const [dados, setDados] = useState<NotificacaoData[]>([
    {
      id: 1,
      titulo: "Exemplo 1",
      mensagem: "Exemplo 2",
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (editingId) {
      ...
      const novoRegistro: NotificacaoData = {
  ...
  const handleEdit = (item: NotificacaoData) => {
    setFormData({
      titulo: item.titulo,
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-slate-100 p-4 md:p-8">
      ...
      <div className="mb-8">
        ...
        <p className="text-slate-600">Gerenciamento de notificações do sistema</p>
      </div>
        ...
        <Dialog open={isDialogOpen} onOpenChange={(open) => {
          ...
          <DialogContent className="max-w-md">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div>
                <Label htmlFor="lida">Lida</Label>
                <Input
                  id="lida"
                  value={formData.lida}
                  onChange={(e) => setFormData({ ...formData, lida: e.target.value })}
                  placeholder="lida"
                />
              </div>
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-slate-200">
                    ...
                    <th className="text-left py-3 px-4 font-semibold text-slate-700">Mensagem</th>
                    ...
                    <th className="text-left py-3 px-4 font-semibold text-slate-700">Lida</th>
      ...
      <td className="py-3 px-4">{item.mensagem}</td>
      <td className="py-3 px-4">{item.tipo}</td>
      <td className="py-3 px-4">{item.data}</td>
      <td className="py-3 px-4">{item.lida}</td>
